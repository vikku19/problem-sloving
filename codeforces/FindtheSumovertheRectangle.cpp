...
vector<vector<int>>findPrefixsum2D(vector<vector<int>>&a){
    int n=a.size();
    int m=a[0].size();
    vector<vector<int>>prefixsum1D(n);
    for(int i=0;i<n;i++){
        prefixsum1D[i]=findPrefixsum(a[i]);
    }
...
void solve(){
    ...
    vector<vector<int>>prefixsum=findPrefixsum2D(a);
    int lx,rx,ly,ry;
    cin>>q;while(q--){
        cin>>lx>>ly>>rx>>ry;
        cout<<prefixsum[rx][ry]-prefixsum[lx][ry]-prefixsum[rx][ly]+prefixsum[lx][ly]<<endl;
    }
}
int main(){
    IOS;
