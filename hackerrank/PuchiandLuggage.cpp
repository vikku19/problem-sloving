...
void merge(int weight[],int start,int mid,int end){
...
void divide(int weight[],int start,int end){
  if(start>=end){
    return;
  }
  int mid=start+(end-start)/2;
  divide(weight,start,mid);
  divide(weight,mid+1,end);
  merge(weight,start,mid,end);
}
void solve(){
    int n;cin>>n;
    int weight[n];
